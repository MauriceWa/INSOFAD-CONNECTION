...
export class CartComponent implements OnInit {
  ...
  public removeTopUpsFromCart(topUp_index: number) {
    this.cartService.removeTopUpFromCard(topUp_index);
  }

  public getTotalPrice(): number {
    let total = this.products_in_cart.reduce((sum, product) => sum + product.price * product.amount, 0);
    ...
    if (this.giftCardCode.trim() !== '') {
      this.giftCardService.validateGiftCard(this.giftCardCode).subscribe(
          (giftCard: GiftCard | null) => {
            console.log('cardApplied', giftCard)
            if (giftCard) {
              console.log('giftcardApplied')
